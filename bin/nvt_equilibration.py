...
"""Run NVT equilibration (gmx grompp -r + gmx mdrun).

Converted from nf-core/moleculardynamics modules/local/run_nvt_equilibration.nf.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run GROMACS NVT equilibration")
    parser.add_argument("--input-gro", required=True, help="Energy-minimized .gro structure")
    parser.add_argument("--input-top", required=True, help="Topology (.top) file")
    parser.add_argument("--mdp", required=True, help="NVT equilibration .mdp file")
    parser.add_argument("--output-gro", required=True, help="Output NVT .gro file")
    ...
    run([args.gmx_cmd, "grompp", "-f", args.mdp, "-c", args.input_gro,
         "-r", args.input_gro, "-p", args.input_top, "-o", tpr])
